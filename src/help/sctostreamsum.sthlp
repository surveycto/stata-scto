...
{synopt :{cmdab:sen:sors(}{it:string}{cmd:)}} Indicates the sensor streams for which basic statistics should be calculated.{p_end}
...
{synopt :{cmdab:llbet:ween(}{it:string}{cmd:)}} Manually specify statistics for the light level stream files.{p_end}
{synopt :{cmdab:slbet:ween(}{it:string}{cmd:)}} Manually specify statistics for the sound level stream files.{p_end}
{synopt :{cmdab:spbet:ween(}{it:string}{cmd:)}} Manually specify statistics for the sound pitch stream files.{p_end}
{synopt :{cmdab:mvbet:ween(}{it:string}{cmd:)}} Manually specify statistics for the movement stream files.{p_end}
...
{pstd}{cmd:sctostreamsum} is a command that calculate statistics from the sensor stream .csv files outputted by the {it:sensor_stream} field in {browse "https://www.surveycto.com/":SurveyCTO's} data collection tool. This command reads .csv files with sensor data (light level, sound level, sound pitch and movement) recorded during the interview by the device used in the data collection. The unit of observation (what each row represents) in the .csv files this command use as input is the time period the sensor data was reported on (the default is one second), but this command outputs a .dta file where the statistics have been collapsed down to one row for each submission so that the .dta file can be merged to the main data.{p_end}
...
{phang}{cmdab:replace} make the command overwrite any file generated by this command already in the {cmdab:output:folder()}. If this option is used, then all .csv files in the {cmdab:media:folder()} will be used regardless if they were in the file that already existed or not. Since this will overwrite the file already in the output folder, all manual edits after the file was originally generated will be lost. Using this option is the only way to update observations in a .dta file already generated using this command.{p_end}
...
{phang}{inp:llbetween(}{it:indoors_lit(100 750)}{inp:)} will create a variable from the light level streams named {it:indoors_lit} where the values are the percentage (in decimal points) of time periods where the mean light level was between 100 lux (exclusive) and 750 lux (exclusive).{p_end}
...
{pstd}This is a very simple way to run the command. The command will read sensor stream .csv files in the media folder with the prefix SL (because {inp:quiet} was used) and the prefix MV (because {inp:still} was used) and add create a .dta file with the key in variable {it:key}, the basic statistics for the sound level sensor and the {it:quiet} variable, and the basic statistics for the movement sensor and the {it:still} variable and save it in the output folder. If the command has already been run and the .dta already exist in the folder, then only submissions with an key not already in the .dta file will be processed and appended to the already existing file. Any sensor stream files with prefix SP or LL in the media folder will be ignored as no statistic applicable to either of those streams was specified.

{pstd} {hi:Example 2.}

{pstd} {inp:sctostreamsum, mediafolder(}{it:}"$media"{inp:) outputfolder(}{it:"$output"}{inp:) quiet slbetween(}{it:loud(60 ?)}{inp:) replace}

{pstd} In this example, the command will only read SL sensor stream .csv files from the media folder as sound level is the only sensor required by the statistics that are specified. The basic statistics will be calculated for the sound level stream, in addition to the {it:quiet} and {it:loud} variables. All SL files in the media folder will be included and the command will start over with a new .dta file and even replace the .dta file in the output folder if it already exist.
