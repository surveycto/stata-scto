...
qui {
	...
	*File exist and should not be replaces
	if !_rc & "`replace'" == "" {
		...
		*Test that the merge file has the variable key needed to merge on
		cap confirm variable key period
		if _rc {
			...
			*There is no variable key. The data set must have been altered. The data set can be alatered as long as the key variable remains the same
			noi di as error `"{phang}The file [`outputFilename'] in the outputfolder(`outputfolder') folder was not generated by this command or was modified and do no longer have the two variables {it:key} and {it:period}. This file can therefore not be used by the command. Use the option {inp:replace} to replace this file. When you replace the file this command re-reads all csv stream files and process them again."'
			error 693
			exit
		}
			...
			*Merging this sensor to other sensors
			if `firstMerge' == 1 {
				...
				*the already processed sensors.
				merge 1:1 key period using `fileToMergeTo', nogen
	...
	*******************************************
	*
	*	Handle same sensor multiple time periods
	*
	*******************************************
