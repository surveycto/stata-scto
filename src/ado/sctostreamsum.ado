...
qui {
	...
	*This segment of code imports the utility functions for these commands
	...
	if (_rc == 0) {
		...
		*File does not exist
		noi di as error "Reinstall package"    // Say that the package should be re-installed using ssc intall sctotools
		error 198
		exit
	}
	...
	*List of names that are taken, it will be updated throughout the command as names are taken
	...
	local outputFilename  "sum_streamstats.dta"
	...
	*File exist and should not be replaces
	if !_rc & "`replace'" == "" {
		...
		*The file exist and should not be replaced. New submissions should be added to the already existing file
		use "`outputfolder'/sctostreamstats_mergefile.dta"
		...
		*Test that the merge file has the variable key needed to merge on
		...
		if !_rc {
			...
			*There is no variable key. The data set must have been altered. The data set can be alatered as long as the key variable remains the same
			noi di as error `"{phang}The sctostreamstats_mergefile.dta file in the outputfolder(`outputfolder') folder does not have been modified and do no longer have the variable key. Use the option replacemergefile and the option updateallcsvs at the same to re-process all files in the media folder and replace all files in the output folder.{p_end}"'
			error 693
			exit
		}

		*List of all the keys already processed
	...
	*The dta file either does not exist or should be replaced. So start from an empty file
		...
		*There are no keys already in the data set
		...
		*Loop over all files
		foreach filecsv of local files`sensorPrefix' {
			...
			*Get file from file key. Do not process this csv file if this obs have already been processed
			...
			//noi di "is `filekey' in `already_processed_keys'"

			loca filecount_output "File `counter' out of `count_files`sensorPrefix'' `sensorPrefix' files."
			...
			*Test if file has already been outputted, and if so, if it should be replaced
			if (`:list filekey in already_processed_keys') {
				...
				*Loop over each between var and label them. If no between var then btwncount is 0.
				forvalues btwnnum = 1/`btwncount' {
	...
	*Save any new observations to the data set
	if `firstMerge' == 1 {

		noi di ""
		noi di "{phang}Command ran successfully but no observations not already in the .dta file was found, so no new observations added to the data set.{p_end}"
